...
async function onSave() {
  ...
  else toast.error(t('toast.saveCancelled'));
}
...
async function onRecentChange(event: Event): Promise<void> {
  const path = (event.target as HTMLSelectElement).value;
  if (!path) return;
  const ok = await store.openRecent(path);
  if (ok) toast.success(t('toast.loaded'));
  else toast.error(t('toast.openFailed'));
...
<template>
  <div class="toolbar">
    ...
    <select class="recent-select" :title="t('toolbar.recent')" @change="onRecentChange">
      <option value="">{{ t('toolbar.recent') }}</option>
      <option v-for="path in config.userConfig.recentFiles" :key="path" :value="path">
        {{ path }}
      </option>
    </select>
    <button
      v-if="config.userConfig.recentFiles.length"
      :title="t('toolbar.clearRecent')"
      @click="config.clearRecentFiles()"
    >
    ...
    <button :title="t('toolbar.saveAs')" @click="onSaveAs">
      {{ t('toolbar.saveAs') }}
    </button>
    <div class="sep" />
    <button
