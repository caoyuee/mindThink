/**
 * MCP 上下文基础模型。
 * 这里不实现传输层，只生成可供 MCP Server 和内置 AI 复用的纯数据。
 */
...
/** 处理一条 stdio/HTTP 行消息，失败时仍返回可传输的 RPC 错误。 */
export function processMcpMessage(line: string, context: McpContext): string {
  try {
    const request = decodeMcpMessage(line);
    if (!('method' in request)) return encodeMcpMessage(request);
    return encodeMcpMessage(handleMcpRpcRequest(request, context));
  } catch (error) {
...
export function parseMcpRequest(value: unknown): McpRequest {
  if (typeof value !== 'object' || value === null) throw new Error('MCP 请求必须是对象');
  const request = value as Record<string, unknown>;
  if (typeof request['requestId'] !== 'string' || typeof request['tool'] !== 'string') {
    throw new Error('MCP 请求缺少 requestId 或 tool');
  }
  const args = request['arguments'];
  if (args !== undefined && (typeof args !== 'object' || args === null || Array.isArray(args))) {
    throw new Error('MCP arguments 必须是对象');
  }
  return {
...
/** 将标准 JSON-RPC 请求解析为内部工具请求。 */
export function parseMcpRpcRequest(value: unknown): McpRpcRequest {
  if (typeof value !== 'object' || value === null) throw new Error('MCP RPC 请求必须是对象');
  const request = value as Record<string, unknown>;
  ...
  try {
    switch (request.method) {
      ...
      case 'resources/read': {
        ...
        if (!uri.startsWith(prefix)) throw new Error('不支持的 MCP resource URI');
        const node = context.nodes[uri.slice(prefix.length)];
        if (!node) throw new Error('MCP 节点不存在');
        return {
          jsonrpc: '2.0',
          id,
          result: {
            contents: [{ uri, mimeType: 'application/json', text: JSON.stringify(node) }],
          },
        };
      }
      case 'tools/call': {
        ...
        return {
          ...
          result: {
            content: [
              {
                type: 'text',
                text: JSON.stringify({ requestId: String(id), tool: name, arguments: args ?? {} }),
              },
            ],
          },
        };
